...
{
    [super viewDidLoad];
	_mapView = [[[MKMapView alloc] initWithFrame:CGRectMake(0, 0, 320, 416)] autorelease];
    ...
    [SVGeocoder geocode:address
             completion:^(NSArray *placemarks, NSError *error) {
                 if(!error && placemarks){
                     ...
                     UIAlertView *av = [[[UIAlertView alloc] initWithTitle:@"地理位置解析错误!" message:nil delegate:nil cancelButtonTitle:@"Ok" otherButtonTitles:nil] autorelease];
                     [av show];
...
- (MKAnnotationView *)mapView:(MKMapView *)mView viewForAnnotation:(id <MKAnnotation>)annotation 
{
    if([annotation isKindOfClass:[BaseAnnotation class]]){
        ...
        MKPinAnnotationView *targetView = [[[MKPinAnnotationView alloc] initWithAnnotation:annotation reuseIdentifier:@"annotationForTarget"] autorelease];   
